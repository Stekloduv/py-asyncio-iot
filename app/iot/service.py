...
class IOTService:
    ...
    async def run_program(self, program: list[Message]) -> None:
        commands_parallel_on = []
        commands_sequence = []
        commands_parallel_off = []
        print("=====RUNNING PROGRAM======")
        for msg in program:
            if msg.msg_type == MessageType.SWITCH_ON:
                commands_parallel_on.append(self.send_msg(msg))
            elif msg.msg_type == MessageType.SWITCH_OFF:
                commands_parallel_off.append(self.send_msg(msg))
            else:
                commands_sequence.append(self.send_msg(msg))

        await asyncio.gather(*commands_parallel_on)

        for command in commands_sequence:
            await command

        await asyncio.gather(*commands_parallel_off)
